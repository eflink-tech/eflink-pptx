 ...
 * 预览取舍：负值统一 clamp 到基线（按 0 处理），不渲染负向柱/面积/折线；
 * 未读取元素级 chartColors/paletteIndex 配色（统一按 Office 默认系列色循环取色）。 */
...
export async function renderChart(node: Element, ctx: PreviewCtx): Promise<SVGElement | null> {
  ...
  if (t === 'pie' || t === 'pie-doughnut') {
    // 饼图/圆环为极坐标类，不画笛卡尔坐标轴
    drawPie(g, chart, box, t === 'pie-doughnut')
  } else {
    drawAxis(g, box)
    if (t.startsWith('bar')) drawBars(g, chart, box, t.includes('horizontal'), t.includes('stack') || t === 'bar-percent')
...
/** 值轴上限：stacked 取各类目累计和的最大值，否则取单点最大值；均 ×1.1 留白，全 0 兜底 1 防除零 */
function valueMax(chart: ChartElement, stacked: boolean): number {
  const rows = chart.data.series.map((s) => s.values)
  const base = stacked
    ? Math.max(0, ...(rows[0] ?? []).map((_, i) => rows.reduce((sum, vs) => sum + Math.max(0, vs[i] ?? 0), 0)))
...
/** 柱状图：类目分带（band）。
 * 纵向：非堆叠按系列均分带宽；堆叠占满整带、各段沿值轴（高度）自下而上累计。
 * 横向与非堆叠对偶：非堆叠沿类目轴均分行高，堆叠占满整行、各段沿值轴（宽度）自左向右累计。 */
function drawBars(g: SVGElement, chart: ChartElement, box: BoxLike, horizontal: boolean, stacked: boolean): void {
  ...
  for (let i = 0; i < n; i += 1) {
    ...
    for (let si = 0; si < series.length; si += 1) {
      // 预览取舍：负值 clamp 到基线（按 0 处理）
      const v = Math.max(0, series[si].values[i] ?? 0)
      const color = CHART_COLORS[si % CHART_COLORS.length]
      if (horizontal) {
        const seg = (v / maxV) * p.w
        if (stacked) {
          // 堆叠：占满整行，段沿值轴向右延伸
          g.appendChild(svgEl('rect', { x: p.x + (acc / maxV) * p.w, y: p.y + band * i, width: seg, height: band, fill: color }))
        } else {
          const y = p.y + band * i + (band / groupSize) * si + (band / groupSize) * 0.1
          g.appendChild(svgEl('rect', { x: p.x, y, width: seg, height: (band / groupSize) * 0.8, fill: color }))
        }
      } else if (stacked) {
        // 堆叠：占满整带，段沿值轴向上延伸
        const h = (v / maxV) * p.h
        g.appendChild(svgEl('rect', { x: p.x + band * i, y: p.y + p.h - ((acc + v) / maxV) * p.h, width: band, height: h, fill: color }))
      } else {
        const x = p.x + band * i + (band / groupSize) * si + (band / groupSize) * 0.1
        g.appendChild(svgEl('rect', { x, y: p.y + p.h - (v / maxV) * p.h, width: (band / groupSize) * 0.8, height: (v / maxV) * p.h, fill: color }))
      }
      acc += v
    }
  }
...
/** 折线图：polyline 逐系列连线；stacked 按低层系列累计取值；marker 时补数据点圆。负值 clamp 到基线。 */
function drawLine(g: SVGElement, chart: ChartElement, box: BoxLike, stacked: boolean, marker: boolean): void {
  ...
  for (let si = 0; si < series.length; si += 1) {
    ...
    for (let i = 0; i < n; i += 1) {
      const v = Math.max(0, series[si].values[i] ?? 0)
        + (stacked ? series.slice(0, si).reduce((s, sr) => s + Math.max(0, sr.values[i] ?? 0), 0) : 0)
...
/** 面积图：闭合 path；stacked 时按系列自下而上累计，绘制顺序反转保证下层可见。负值 clamp 到基线。 */
function drawArea(g: SVGElement, chart: ChartElement, box: BoxLike, stacked: boolean): void {
  ...
  for (const si of order) {
    ...
    const d = `M ${pts.join(' L ')} L ${p.x + p.w},${p.y + p.h} L ${p.x},${p.y + p.h} Z`
    g.appendChild(svgEl('path', { d, fill: CHART_COLORS[si % CHART_COLORS.length], 'fill-opacity': stacked ? 1 : 0.5 }))
  }
}

/** 散点图：labels 为 xVal 数值串，系列 values 为 y；按数据范围线性映射到绘图区 */
...
function drawPie(g: SVGElement, chart: ChartElement, box: BoxLike, doughnut: boolean): void {
  ...
  for (let i = 0; i < vals.length; i += 1) {
    ...
    if (a1 - a0 >= Math.PI * 2 - 1e-9) {
      // 单片满圆：弧命令退化，改画整圆（圆环用居中描边环）
      if (rIn <= 0) {
        g.appendChild(svgEl('circle', { cx, cy, r, fill: color }))
      } else {
        g.appendChild(svgEl('circle', { cx, cy, r: (r + rIn) / 2, fill: 'none', stroke: color, 'stroke-width': r - rIn }))
      }
    } else {
      g.appendChild(svgEl('path', { d: slicePath(cx, cy, r, rIn, a0, a1), fill: color }))
    }
    a0 = a1
  }
}
